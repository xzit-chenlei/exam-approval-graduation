...
import edu.xzit.graduate.service.CourseAbilityLabService;
import lombok.Data;
...
/**
 * 毕业达成度课程能力交叉(实验课)Controller
 *
 * @author vikTor
 * @date 2025-09-21
 */
@RestController
@RequestMapping("/graduateLab/matrix")
public class GraduateCourseAbilityLabController extends BaseController {

    @Autowired
    private CourseAbilityLabService courseAbilityLabService;

    /** 查询课程×能力矩阵 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:query')")
    @GetMapping("/list")
    public AjaxResult list(@RequestParam Long majorId, @RequestParam Integer grade) {
        return success(courseAbilityLabService.getMatrix(majorId, grade));
    }

    /** 新增行：课程 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:addRow')")
    @Log(title = "毕业达成度矩阵-新增课程", businessType = BusinessType.INSERT)
    @PostMapping("/row")
    public AjaxResult addRow(@RequestBody GraduateCourse course) {
        return success(courseAbilityLabService.addCourse(course));
    }

    /** 修改行：课程 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:editRow')")
    @Log(title = "毕业达成度矩阵-修改课程", businessType = BusinessType.UPDATE)
    @PutMapping("/row")
    public AjaxResult editRow(@RequestBody GraduateCourse course) {
        courseAbilityLabService.updateCourse(course);
        return success();
    }

    /** 删除行：课程 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:removeRow')")
    ...
    public AjaxResult removeRow(@PathVariable Long courseId) {
        courseAbilityLabService.deleteCourse(courseId);
        return success();
    }

    /** 新增列：能力 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:addCol')")
    @Log(title = "毕业达成度矩阵-新增能力", businessType = BusinessType.INSERT)
    @PostMapping("/col")
    public AjaxResult addCol(@RequestBody GraduateAbility ability) {
        return success(courseAbilityLabService.addAbility(ability));
    }

    /** 修改列：能力 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:editCol')")
    @Log(title = "毕业达成度矩阵-修改能力", businessType = BusinessType.UPDATE)
    @PutMapping("/col")
    public AjaxResult editCol(@RequestBody GraduateAbility ability) {
        courseAbilityLabService.updateAbility(ability);
        return success();
    }

    /** 删除列：能力 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:removeCol')")
    ...
    public AjaxResult removeCol(@PathVariable Long abilityId) {
        courseAbilityLabService.deleteAbility(abilityId);
        return success();
    }

    /** 设置或更新单元格 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:editCell')")
    @Log(title = "毕业达成度矩阵-设置单元格", businessType = BusinessType.UPDATE)
    @PostMapping("/cell")
    public AjaxResult upsertCell(@RequestBody CourseAbilityController.CellDTO dto) {
        courseAbilityLabService.upsertCell(dto.getCourseId(), dto.getAbilityId(), dto.getGrade(), dto.getLevel(), dto.getWeight(), dto.getRemark());
        return success();
    }

    /** 清除单元格 */
    @PreAuthorize("@ss.hasPermi('graduateLab:matrix:clearCell')")
    ...
    public AjaxResult clearCell(@RequestParam Long courseId, @RequestParam Long abilityId) {
        courseAbilityLabService.clearCell(courseId, abilityId);
        return success();
    }
