...
import edu.xzit.core.core.flowable.domain.dto.FlowSaveXmlVo;
...
public class FlowDefinitionController extends BaseController {
    ...
    public AjaxResult list(@ApiParam(value = "当前页码", required = true) @RequestParam Integer pageNum,
                           ...
                           @ApiParam(value = "是否模糊查询", required = false) @RequestParam(required = false) Boolean fuzzyQuery
    ) {
        return AjaxResult.success(flowDefinitionService.list(name, fuzzyQuery, pageNum, pageSize));
    }
    ...
    public AjaxResult importFile(@RequestParam(required = false) String name,
        ...
        try {
            ...
            flowDefinitionService.importFile(name,null, category, in);
        } catch (Exception e) {
            log.error("导入失败:", e);
    ...
    public AjaxResult save(@RequestBody FlowSaveXmlVo vo) {
        ...
        try {
            ...
            flowDefinitionService.importFile(vo.getName(), vo.getKey(), vo.getCategory(), in);
        } catch (Exception e) {
            log.error("导入失败:", e);
    ...
    @ApiOperation(value = "流程定义字典列表")
    public AjaxResult flowNodeList() {
        return AjaxResult.success(flowDefinitionService.getFlowNodeList());
    }
