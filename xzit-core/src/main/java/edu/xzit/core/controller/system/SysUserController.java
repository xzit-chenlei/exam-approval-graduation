...
public class SysUserController extends BaseController {
    ...
    public TableDataInfo list(SysUser user) {
        startPage();
        List<SysUser> list = userBizService.queryUserList(user);
        return getDataTable(list);
    }

    /**
     * 获取用户列表（排除管理员ID=1），用于人员选择
     */
    @GetMapping("/listExcludeAdmin")
    public TableDataInfo listExcludeAdmin(SysUser user) {
        startPage();
        List<SysUser> list = userBizService.queryUserListExcludeAdmin(user);
        return getDataTable(list);
    }
     ...
     */
    @GetMapping("/listInfo")
    public AjaxResult listInfo(SysUser user) {
        List<SysUser> list = userService.selectUserListInfo(user);
        return success(list);
    }
